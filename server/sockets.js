...
function onConnection(socket) {
	...
	socket.on('registerPlayer', function (data, res) {
		...
		var currentGame = gameManager.getGameById(data.gameId);

		if (!currentGame)
			return res({ error: 'The specified game does not exist, can\'t join.'});
		if (!currentGame.checkKey(data.key))
	...
	socket.on('command', function (data, res) {
		...
		if (!currentGame || !currentGame.hasPlayer(player)) {
			return;
		}

		currentGame.sendCommand(data.command, player);
	});
